...
private:
     ...
     struct Node
     {
          Node(Key key, Info info, Node *prev, Node *next)
          {
               this->element = new Element(key, info);
               this->prev = prev;
               this->next = next;
          }
          Element *element;
...
bi_ring<Key, Info> shuffle(const bi_ring<Key, Info> &first, unsigned int fcnt, const bi_ring<Key, Info> &second, unsigned int scnt, unsigned int reps)
{
     ...
     if (second.empty())
          scnt = 0;
     typename bi_ring<Key, Info>::const_iterator fit = first.cbegin();
     typename bi_ring<Key, Info>::const_iterator sit = second.cbegin();
